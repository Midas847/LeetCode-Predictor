...
router.get("/", async (req, res) => {
  try {
    ...
    for(const item of response_each.data.total_rank){
        let user_name="uwi";
        let query_url='https://leetcode.com/graphql?query=query{userContestRanking(username: "ayushman_sinha"){rating}}'
        const response_rating=await axios.get(query_url);

        username_list.push(item.username+" "+response_rating.data.data.userContestRanking.rating);
    }
